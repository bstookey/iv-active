	...
	<nav aria-label="Footer Social Navigation">
		<ul id="footer-social" class="footer-social social-icons" role="menu-bar">
			...
			foreach ($social_networks as $network) :
				...
				$network_url = get_theme_mod('theme_' . $network . '_link');

				// Only display the list item if a URL is set.
				if (!empty($network_url)) :
			?>
					<li class="<?= esc_attr($network) ?> menu-item">
						<a href="<?php echo esc_url($network_url); ?>" title="<?php echo ucwords(esc_attr($network)); ?> " class="social-icon <?php echo esc_attr($network); ?>" target="_blank">
							<?php
							print_svg(
								[
									'icon'   => $network,
							...
							<span class="screen-reader-text">
								...
								printf(esc_attr__('Go to %s', THEME_DOMAIN), ucwords(esc_html($network))); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
								?>
							</span>
						</a>
					</li><!-- .social-icon -->
